		...
		try {
			...
			if (userBean.getBelong().equals("admin")) {
				...
				if (path.equals("/main.adm")) {
					...
					req.setAttribute("manageStuMonth", dao.getManageStuMonth(yyyymm));
		...
		try {
			...
			if (userBean.getBelong().equals("admin")) {
				...
				if (path.equals("/manage_lec_update.adm")) {
					...
					int idx = dao.updateLecture(lectureBean);
					...
					int idx = dao.insertLecture(lectureBean);
					rd = req.getRequestDispatcher("manage_lec_detail.adm?idx="+idx);
					...
					idx = dao.deleteLecture(idx); //나중에 쓸 결과값
					...
					int idx = dao.updateManageStuMonth(yyyymm,userId,status);
					
					rd = req.getRequestDispatcher("manage_stu_month.adm?idx="+idx);
				} else if (path.equals("/manage_stu_month_update.adm")) {
					// 수강생관리 목록 페이지 삭제
					String[] userId = req.getParameterValues("");
					int idx = dao.deleteUser(userId);
...
//					teacherBean.set어쩌구
					int idx = dao.insertTeacher(teacherBean);
					...
					TeacherDto teacherBean = new TeacherDto();
//					teacherBean.set어쩌구
					int idx = dao.updateTeacher(teacherBean);
					...
					int idx = dao.updateQnaL(answerContent);
					rd = req.getRequestDispatcher("qna_detail.adm?idx="+idx);

				} else if (path.equals("/qna_delete.adm")) {
					// 큐엔에이 삭제 페이지
					String[] qnaLIdList = req.getParameterValues("");
					int idx = dao.deleteQnaL(qnaLIdList);
					rd = req.getRequestDispatcher("qna.adm");
				} else if (path.equals("/main.adm")) {
					// 일정상세보기 페이지 (모달창)
					String calendarId = req.getParameter("");
					req.setAttribute("modalCalendar", dao.getCalendar(calendarId));
					rd = req.getRequestDispatcher("main.adm");
				} 
			}
		} catch (java.lang.NullPointerException e) {
			resp.sendRedirect("login.bit");
